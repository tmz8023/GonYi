...
<body>
	...
	<div id="content">
		...
		<div class="m-right">
			...
			<form action="book_updateren.action"method="post">
			<table class="table">
				<tr>
					<td><input type="checkbox" name="expend">
					</td>
					<td>项目编号</td>
					<td>申请项目名称</td>
					<td>申请时间</td>
				...
				<c:forEach items="${ren}" var ="r">
					...
					<td>${r.nstatus }</td>
					<td>${r.shenname}</td>
					<td><input type="radio" value="审核通过" name="opinion" />同意 <input
								type="radio" value="审核未通过" name="opinion" />不同意
					</td>
					<td><textarea name="reasons" cols=10 rows=1></textarea></td>
					<td><input type="submit"  value="审核完成"></td>
					</tr>
				</c:forEach>
				</tbody>
				</table>
