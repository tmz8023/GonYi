					...
					<div class="col-lg-6">
						<div class="input-group" style="float:left">
							...
							<div style="float:left">
								<button onclick="clickChooseReverse()" class="btn btn-info">全选</button>
							</div>
							...
							<div style="margin:0px 303px;">
								<a href="en_fenye.action">
									<button type="button" class="btn btn-primary">查询全部</button> </a>
							</div>
						</div>
					</div> -->
					<div style="height:350px;">
					<form action="en_Faqiservlet.action" method="post">
						<table border="1" class="table table-bordered"
							style="margin-left:0px;margin-right:5px;">
							<tr>
								<!-- <td><input type="checkbox" class="choose-all-input"
									onclick="clickChooseAllInput()" /></td> -->
								<td>资金表编号</td>
								<td>项目编号</td>
								<td>项目名称</td>
								<td>审核部审核时间</td>
							...
							<c:forEach items="${List.rows}" var="pp">
								<tr>
									...
									<td><input type="text" size="5" name="xm_name"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.xm_name}"></td>
									<td><input type="text" size="5" name="sh_status"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.sh_status }"></td>
									<td><input type="text" size="5" name="sh_time"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.sh_time}"></td>
									<td><input type="text" size="5" name="sh_reasons"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.sh_reasons}"></td>
									<td><input type="text" size="5" name="sh_person"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.sh_person}"></td>
									<td><input type="radio" value="审核通过" name="ls_status" />同意
										<input type="radio" value="审核未通过" name="ls_status" />拒绝</td>
									<td><input type="text" size="5" name="lz_person1"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="史志豪">
									</td>
									<td><textarea name="note" cols=8 rows=1 id="reason"></textarea>
									</td>
									<td><input type="text" name="sh_money" size="5"
										value="${pp.sh_money}" id="money"style="border:0px;background:rgba(0, 0, 0, 0);">
									</td>
									<td><input type="text" name="moneym" size="5"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.moneym}" id="r2" />
									</td>
									<td><input type="text"  size="5"
										style="border:0px;background:rgba(0, 0, 0, 0);"
										value="${pp.shengxia}" id="r3" />
									</td>
									<td><input type="text" name="yimoney" size="5"
										style="border:1px;"
										value="${pp.moneym}" id="r1" />
									</td>
									
									<td><button type="button"
											onclick="xiangqing('${pp.xm_name}')"
											class="btn default btn-xs" data-toggle="modal"
											data-target=".bs-example-modal-lg" id="q1">详情</button>&nbsp;
										<button type="submit" class="btn default btn-xs" id="sb">审核</button></td>
								</tr>
							</c:forEach>
						</table>
					</form>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal" id="sure">确定</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		$("#sb").click(function(){
			...
			if(reason.length==0){
				alert("请输入审批金额或审批意见");
				return false;
			}else if(parseFloat(r2)<parseFloat(r1)){
				alert("批准金额不能超过申请金额");
				return false;
			}else{
				alert("审核完成");
