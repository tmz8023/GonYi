package com.YXcexpend;


public class ZhiChu {
	 private int expend_askid;
	 private int expend_name;
	 private String ask_name;
	 private int expend_money;
	 private String expend_time;
	 private String expend_perspon;
	 private String expend_status;
	 
	 
	
	public String getAsk_name() {
		return ask_name;
	}
	public void setAsk_name(String ask_name) {
		this.ask_name = ask_name;
	}
	public String getExpend_status() {
		return expend_status;
	}
	public void setExpend_status(String expend_status) {
		this.expend_status = expend_status;
	}
	public int getExpend_askid() {
		return expend_askid;
	}
	public void setExpend_askid(int expend_askid) {
		this.expend_askid = expend_askid;
	}
	public int getExpend_name() {
		return expend_name;
	}
	public void setExpend_name(int expend_name) {
		this.expend_name = expend_name;
	}
	public int getExpend_money() {
		return expend_money;
	}
	public void setExpend_money(int expend_money) {
		this.expend_money = expend_money;
	}
	
	public String getExpend_time() {
		return expend_time;
	}
	public void setExpend_time(String expend_time) {
		this.expend_time = expend_time;
	}
	public String getExpend_perspon() {
		return expend_perspon;
	}
	public void setExpend_perspon(String expend_perspon) {
		this.expend_perspon = expend_perspon;
	}
	 
}
